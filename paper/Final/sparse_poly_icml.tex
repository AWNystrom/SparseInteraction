...
For a vector of dimension $D$ and density $0 \le d \le 1$ the algorithm has time complexity $O(d^kD^k)$ where $k$ is the polynomial-feature order; this  is an improvement by a factor $d^k$ over the standard method.
\end{abstract}
...
The instance-independent nature of feature mappings allows them to pair well with linear parameter estimation techniques such as stochastic gradient descent, making them a candidate for certain types of large-scale machine learning problems when $D < N$.
