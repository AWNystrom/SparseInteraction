...
matrix of size $N \times D$ with $O(d^kD^kN)$ where $k$ is the polynomial order and $d$ is the density of each row.
...
where $(x_{i_0}, x_{i_1}, \dots, x_{i_{k-1}})$ are column indicies of a row vector $\vec{x}$ of an $N \times D$ input matrix and $p_{i_0i_1 \dots i_{k-1}}$ is a column index of the polynomial expansion matrix.
